...
def public_factual_accuracy_command(
    ...
    return [
        ...
        "--max-missing-rate",
        str(baseline["maxMissingRate"]),
        "--max-mismatch-rate",
        str(baseline["maxMismatchRate"]),
        "--output-json",
...
def verify_public_meta(public_dir: Path, expected_base_path: str) -> None:
    ...
    repositories_document = repositories_path.read_text()
...
def smoke_test_release_bundle(
    ...
    with tempfile.TemporaryDirectory(prefix="dotrepo-smoke-") as tmp:
        ...
        try:
            ...
            if status != 200:
                raise SystemExit(
                    f"same-origin repository catalog smoke failed ({status}) for "
