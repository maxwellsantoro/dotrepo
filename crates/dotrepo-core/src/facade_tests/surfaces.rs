...
#[test]
fn generate_check_ignores_and_generate_preserves_stamp_only_version_drift() {
    let root = temp_dir("stamp-only-version-drift");
    fs::write(
        root.join(".repo"),
        r#"
schema = "dotrepo/v0.1"
...
"#,
    ...
    let document = load_manifest_document(&root).expect("manifest document loads");
    let outputs =
        managed_outputs(&root, &document.manifest, &document.raw).expect("managed outputs render");
    assert_eq!(outputs.len(), 5);
    let current_version = format!("generated by dotrepo {}", crate::render::GENERATOR_VERSION);
    for (path, contents) in outputs {
        let old_stamp = contents.replacen(&current_version, "generated by dotrepo 1.0.1", 1);
        assert_ne!(old_stamp, contents);
        if let Some(parent) = path.parent() {
            fs::create_dir_all(parent).expect("output parent created");
        }
        fs::write(path, old_stamp).expect("old-version output written");
    }
    ...
    assert!(report.outputs.iter().all(|output| {
        !output.stale
            && output.current.is_none()
            && output.expected.contains("generated by dotrepo 1.0.1")
    }));

    let regenerated = managed_outputs(&root, &document.manifest, &document.raw)
        .expect("managed outputs render again");
    for (path, contents) in regenerated {
        assert_eq!(
            contents,
            fs::read_to_string(path).expect("existing output remains readable")
        );
        assert!(contents.contains("generated by dotrepo 1.0.1"));
    }

    let readme_path = root.join("README.md");
    let tampered = fs::read_to_string(&readme_path)
        .expect("README readable")
        .replacen("Fast local-first sync engine", "Tampered content", 1);
    fs::write(&readme_path, tampered).expect("README tampered");

    let report = generate_check_repository(&root).expect("generate check succeeds");
    assert_eq!(report.stale, vec!["README.md"]);
