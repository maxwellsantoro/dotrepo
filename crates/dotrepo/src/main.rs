// Copy of dotrepo-cli/src/main.rs: this alias must compile against the
// published dotrepo-cli lib API, which does not yet expose a shared entry
// point. Keep in sync until one exists, then collapse both mains onto it.
